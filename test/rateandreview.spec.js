...
describe.only('Rate and Review Accomodation/', () => {
    it('login requester', (done) => {
      ...
      it('Should add a rating', (done)=>{
        chai.request(app).post('/api/v1/rating/rate')
      ...
      it('Should get all ratings provided', (done)=>{
        chai.request(app).get('/api/v1/rating/accomodationratings')
            .send({accomodationId: 1, rating: 1})
            .set({ "Authorization": `Bearer ${requesterToken}` })
            .end((err, response)=>{
                expect(response).to.have.status(200);
      ...
      it('Should add a review', (done)=>{
        chai.request(app).post('/api/v1/review/review')
      ...
      it('Should get all ratings provided', (done)=>{
        chai.request(app).get('/api/v1/review/allreviews')
            .set({ "Authorization": `Bearer ${requesterToken}` })
            .end((err, response)=>{
                expect(response).to.have.status(200);
                done();
